...
export default function UploadZone({ onUpload, uploading, uploadMsg }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <svg className={`w-4 h-4 ${uploading ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          {uploading ? (
      ...
      {showToast && uploadMsg && (
        <div className="absolute top-full mt-2 right-0 z-50 animate-slide-down min-w-[200px]">
          ...
          >
            <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${uploadMsg.type === 'success' ? 'bg-green-500' : 'bg-red-500'}`} />
            {uploadMsg.text}
          </div>
        </div>
      )}
